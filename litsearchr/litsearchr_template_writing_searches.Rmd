...
```{r}
study_data <- litsearchr::import_results(directory=scoping_directory, filename=NULL,
                                       remove_duplicates = FALSE, 
                                       clean_dataset = TRUE)

study_data <- litsearchr::deduplicate(study_data, use_abstracts = FALSE, use_titles = TRUE, method = "quick")
study_data <- litsearchr::deduplicate(study_data, use_abstracts = FALSE, use_titles = TRUE, method = "tokens", title_sim=.95)
study_data <- litsearchr::deduplicate(study_data, use_abstracts = TRUE, use_titles = TRUE, method = "tokens", doc_sim = .85)

raked_keywords <- litsearchr::extract_terms(study_data, type="RAKE",
                                            new_stopwords = NULL, 
                                            min_freq = 2, 
                                            title = TRUE, 
                                            abstract = TRUE, 
...
real_keywords <- litsearchr::extract_terms(study_data, type = "tagged",
                                                 ngrams = TRUE, n=2)

study_dict <- litsearchr::make_dictionary(terms = list(raked_keywords, real_keywords))
